...
def update_user_setting():
    ...
    try:
        ...
        setting_mapping = {
            'showKoboToggle': 'show_kobo',
            'incognitoModeToggle': 'incognito_mode',
            'appSoundsToggle': 'app_sounds',
            'fingerprintPasswordToggle': 'fingerprint_password',
            'fingerprintPinToggle': 'fingerprint_pin',
            'hideSensitiveDataToggle': 'hide_sensitive_data',
            'themeToggle': 'theme',
        ...
        backend_setting = setting_mapping[setting_name]
        
        # Handle different value types
        if backend_setting in ['show_kobo', 'incognito_mode', 'app_sounds', 'fingerprint_password', 'fingerprint_pin', 'hide_sensitive_data']:
            current_settings[backend_setting] = bool(value)
        elif backend_setting == 'theme':
            current_settings[backend_setting] = 'dark' if value else 'light'
        elif backend_setting in ['language', 'layout']:
            current_settings[backend_setting] = str(value)
        
        # Save settings
        if save_user_settings(user_id, current_settings):
            # Update session immediately
            if backend_setting == 'language':
                session['lang'] = current_settings[backend_setting]
                session.modified = True
            elif backend_setting == 'theme':
                session['dark_mode'] = current_settings[backend_setting] == 'dark'
                session.modified = True
...
@requires_role(['trader', 'startup', 'admin'])
def preferences():
    """Display user preferences page with all settings."""
    try:
        ...
        if not user_settings:
            from user_settings import get_default_user_settings
            user_settings = get_default_user_settings()
        
        logger.info(
            f"Rendering preferences page for user {user_id}",
            extra={'session_id': session.get('sid', 'no-session-id'), 'user_id': current_user.id}
        )
        ...
        return render_template(
            ...
            title=trans('settings_preferences_title', default='User Preferences', lang=session.get('lang', 'en'))
        )
        
    except Exception as e:
        logger.error(
            f"Error loading preferences for user {current_user.id}: {str(e)}",
            exc_info=True, extra={'session_id': session.get('sid', 'no-session-id'), 'user_id': current_user.id}
        )
        flash(trans('general_something_went_wrong', default='An error occurred'), 'danger')
