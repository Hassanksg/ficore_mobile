...
def auto_optimize_on_startup(app):
    """Automatically run optimizations on app startup."""
    try:
        logger.info('Starting automatic optimization on startup', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
        ...
        try:
            ...
            db = get_mongo_db()
            db_optimizer.optimize_collection_indexes(db)
            logger.info('Database indexes optimized automatically', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
        except Exception as e:
            logger.warning(f'Database optimization failed: {e}', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
        
        # Optimize static assets if in production
        if os.getenv('FLASK_ENV') == 'production':
            try:
                optimizer = static_optimizer
                # Only create bundles if they don't exist
                css_bundle_path = os.path.join(app.static_folder, 'css', 'bundle.min.css')
                js_bundle_path = os.path.join(app.static_folder, 'js', 'bundle.min.js')
                
                if not os.path.exists(css_bundle_path) or not os.path.exists(js_bundle_path):
                    result = optimizer.optimize_all_assets()
                    logger.info(f'Static assets optimized: CSS={result.get("css_bundle")}, JS={result.get("js_bundle")}', 
                               extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
                else:
                    logger.info('Static asset bundles already exist, skipping optimization', 
                               extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
            except Exception as e:
                logger.warning(f'Static asset optimization failed: {e}', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
        
        logger.info('Automatic optimization completed', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
        
    except Exception as e:
        logger.error(f'Auto-optimization failed: {e}', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
...
def create_app():
    ...
    app.config['PERFORMANCE_CACHE_TIMEOUT'] = int(os.getenv('PERFORMANCE_CACHE_TIMEOUT', 3600))
    app.config['PERFORMANCE_STATIC_CACHE_TIMEOUT'] = int(os.getenv('PERFORMANCE_STATIC_CACHE_TIMEOUT', 2592000))
    app.config['PERFORMANCE_ENABLE_GZIP'] = os.getenv('PERFORMANCE_ENABLE_GZIP', 'true').lower() == 'true'
    app.config['PERFORMANCE_ENABLE_ETAG'] = os.getenv('PERFORMANCE_ENABLE_ETAG', 'true').lower() == 'true'
    ...
    # Configure compression based on environment
    if app.config.get('PERFORMANCE_ENABLE_GZIP', True):
        ...
        logger.info('Compression enabled', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
    ...
    try:
        ...
        logger.info('Performance optimizations initialized', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
    except Exception as e:
        logger.warning(f'Performance optimizations failed to initialize: {e}', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
    ...
    try:
        with app.app_context():
            ...
            # Auto-optimize on startup in production
            if os.getenv('FLASK_ENV') == 'production' or os.getenv('AUTO_OPTIMIZE', 'false').lower() == 'true':
                auto_optimize_on_startup(app)
    ...
    from admin_receipt_routes import admin_receipt_bp
    ...
    logger.info('Registered all blueprints including KYC, Settings, Language, and Admin Receipt Management', extra={'session_id': 'none', 'user_role': 'none', 'ip_address': 'none'})
    ...
    # Enhanced language persistence and user settings loading
    @app.before_request
    def load_user_settings_and_language():
        # Load user settings if authenticated
        if current_user.is_authenticated:
            try:
                from user_settings import apply_user_settings_to_session
                apply_user_settings_to_session(current_user.id)
            except Exception as e:
                logger.warning(f'Error loading user settings: {str(e)}', extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        
        # Ensure language is set if not already
        if 'lang' not in session:
            # Priority order: user preference -> browser language -> default
            if current_user.is_authenticated:
                try:
                    user_lang = current_user.get('language')
                    if user_lang and user_lang in ['en', 'ha']:
                        session['lang'] = user_lang
                        session.modified = True
                        return
                except Exception as e:
                    logger.warning(f'Error getting user language preference: {str(e)}', extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            
            # Fallback to browser language or default
            browser_lang = request.accept_languages.best_match(['en', 'ha'], 'en')
    ...
    # Critical CSS and JS functions with fallbacks
    def get_critical_css():
        ...
        try:
            ...
            if os.path.exists(critical_css_path):
                ...
                return """
                :root{--navbar-height:80px;--background-main:#F2EFEA;--text-color:#1e293b;--card-bg:#f9f9f9;}
                .dark-mode{--background-main:#0f172a;--text-color:#f1f5f9;--card-bg:#1e293b;}
                *{box-sizing:border-box;}
                html,body{margin:0;padding:0;font-family:'Poppins',sans-serif;background-color:var(--background-main);color:var(--text-color);}
                .fintech-top-header{position:fixed;top:0;width:100%;height:var(--navbar-height);z-index:1000;}
    ...
    def get_critical_js():
        ...
        try:
            critical_js_path = os.path.join(app.static_folder, 'js', 'critical.js')
            if os.path.exists(critical_js_path):
                with open(critical_js_path, 'r', encoding='utf-8') as f:
                    return f.read()
            else:
                # Return basic critical JS if file doesn't exist
                return """
                (function(){
                    ...
                    if('serviceWorker' in navigator){
                        window.addEventListener('load',()=>{
                            navigator.serviceWorker.register('/static/service-worker.js').catch(e=>console.log('SW registration failed:',e));
                        });
                    }
                })();
                """
    ...
    def get_asset_url(filename):
        ...
        try:
            if hasattr(static_optimizer, 'asset_url'):
                return static_optimizer.asset_url(filename)
        except:
            pass
    ...
    app.jinja_env.globals.update(
        ...
        should_show_subscription_banner=lambda: utils.should_show_subscription_banner(current_user) if current_user.is_authenticated else False,
        critical_css=get_critical_css,
        critical_js=get_critical_js,
        ...
        try:
            ...
            metrics = {
                ...
                'application': {
                    ...
                    'compression_enabled': app.config.get('COMPRESS_REGISTER', True),
                    'cache_timeout': app.config.get('PERFORMANCE_CACHE_TIMEOUT', 3600)
                }
            }
            ...
            logger.error(f'Performance metrics failed: {str(e)}', extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
    ...
    def add_performance_headers(response):
        ...
        try:
            ...
            if request.path.startswith('/static/'):
                ...
                # Add ETag for static files if enabled
                if app.config.get('PERFORMANCE_ENABLE_ETAG', True) and response.data:
                    import hashlib
